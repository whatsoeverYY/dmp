...
export const editPrompts =
  ...
  '导出一个component，名称为{name}EditList。\n' +
  '在{待补充数据}处根据下面提供的表格完善代码，规则如下:\n' +
  'formItemConfig数组值来自于检索字段表格中的[E_转大写下划线{name}_SEARCH_PARAMS].[大写(检索字段)]，' +
  '以及4个来自E_BASE_SEARCH_PARAMS的基本字段，包括E_BASE_SEARCH_PARAMS.CREATED_BY等。。\n' +
  ...
  '{待补充数据示例：E_{转下划线大写(name)}_SEARCH_PARAMS.XX_XX},\n' +
  ...
  '];\n' +
  'const columns = [\n' +
  '{待补充数据示例：E_{转下划线大写(name)}_LIST_COLUMNS.XX_XX},\n' +
  ...
  'columns={columns}\n' +
  'defaultDataStatusList={[DATA_STATUS.EDITING, DATA_STATUS.IN_RELEASE]}\n' +
  'overrideEmptyParams={{\n' +
  '  data_status_list: [DATA_STATUS.EDITING, DATA_STATUS.IN_RELEASE],\n' +
  '}}\n' +
  'rowId="id"\n' +
  'checkMethod={(item) =>\n' +
  '  [\n' +

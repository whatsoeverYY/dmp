...
export const typePrompt =
  ...
  '导出三个接口，分别为：\n' +
  '接口{name}Dto，这个接口继承IBaseEntityInfoV2，接口中的字段名为列表字段表格中中的列表字段，类型为列表字段表格中中的数据类型值，每个字段均是可选字段。\n' +
  '接口{name}SearchParams，这个接口继承BaseSearchParamsV2，接口中的字段名为下表中的检索字段，类型为下表中的检索类型值。\n' +
  '如果某字段的备注值为DATE，则生成两个字段，字段名为[检索字段][_from]以及[检索字段][_to]，类型均为string | number。\n' +
  '每个字段均是可选字段。\n' +
  '接口{name}SearchFormParams，这个接口继承BaseSearchFormParams，接口中的字段名为[fe_][检索字段]，类型为下表中的展示数据值。\n' +
